...
class GPT2Block(nn.Module):
    ...
    def forward(
        ...
        use_cache: Optional[bool] = False,
        output_attentions: Optional[bool] = False,
        layer_mask: Optional[torch.FloatTensor] = None,
        ...
        if layer_mask is not None:
            hidden_states = hidden_states * layer_mask[0]
        ...
        if layer_mask is not None:
            attn_output = attn_output * layer_mask[1]
        ...
        if layer_mask is not None:
            hidden_states = hidden_states * layer_mask[2]
        
        feed_forward_hidden_states = self.mlp(hidden_states)

        if layer_mask is not None:
            feed_forward_hidden_states = feed_forward_hidden_states * layer_mask[3]
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        language_selective_mask: Optional[torch.FloatTensor] = None,
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if language_selective_mask is not None:
                hidden_states = hidden_states * language_selective_mask[i]

            # language_selective_mask[i*4:(i+1)*4] if language_selective_mask is not None else None
